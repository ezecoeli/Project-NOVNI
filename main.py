...
soldier_death_fx = pygame.mixer.Sound("assets/audio/sounds/soldier_death.mp3")
soldier_death_fx.set_volume(0.2)
...
# Itembox respawn function
def check_item_respawn():
    current_time = pygame.time.get_ticks()
    for item_data in list(respawn_timers):
        item_type, x, y, respawn_time = item_data
        if current_time >= respawn_time:
            new_item = ItemBox(item_type, x, y)
            item_box_group.add(new_item)
            respawn_timers.remove(item_data)
...
class Soldier(pygame.sprite.Sprite):
    def __init__(self, char_type, x, y, scale, speed, ammo, grenades, shoot_cooldown_time=20):
        ...
        self.idling = False
        self.idling_counter = 0
        self.health_bar = SoldierHealthBar(self)
...
class ItemBox(pygame.sprite.Sprite):
    def __init__(self, item_type, x, y):
        ...
        self.rect = self.image.get_rect()
        self.rect.midtop = (x + TILE_SIZE // 2, y + (TILE_SIZE - self.image.get_height()))
        self.initial_y = y  # Guardamos la posición inicial correctamente
        self.initial_x = x
    ...
    def update(self):
        ...
        if pygame.sprite.collide_rect(self, player):
            ...
            respawn_timers.append((self.item_type, self.initial_x, self.initial_y, pygame.time.get_ticks() + 20000))
            # Delete item box
            self.kill()

# Alien Health bar class
class HealthBar():
...
class BossHealthBar():
    ...
    def draw(self, surface):
        ...
        # Draw Boss Name
        boss_name = "Dr. Metroid"
        text_surface = font.render(boss_name, True, (255, 255, 255))
        text_rect = text_surface.get_rect(center=(self.x + self.width // 2, self.y - 15))
        surface.blit(text_surface, text_rect)

# Soldiers Health Bar class
class SoldierHealthBar():
    ...
    def draw(self, surface):
        if self.enemy.alive:
            ...
            health_color = (39, 174, 96)  # Verde

            # Cambiar de color según la salud
            if self.enemy.health <= self.enemy.max_health * 0.6:
                health_color = (255, 165, 0)  # Naranja
            if self.enemy.health <= self.enemy.max_health * 0.3:
                health_color = (255, 0, 0)  # Rojo

            # Posición de la barra (encima del enemigo)
            x = self.enemy.rect.centerx - self.width // 2
            y = self.enemy.rect.top - 10

            # Dibujar barra de fondo
            pygame.draw.rect(surface, (50, 50, 50), (x, y, self.width, self.height))
            # Dibujar barra de vida
            pygame.draw.rect(surface, health_color, (x, y, self.width * ratio, self.height))
...
# Function to manage the path of csv files
...
csv_path = resource_path(f"assets/levels/level{level}_data.csv")
